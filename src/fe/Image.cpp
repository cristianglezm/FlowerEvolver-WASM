...
namespace fe{
    ...
    , mWidth(0)
    , mHeight(0){}
    void Image::create(std::size_t width, std::size_t height, const fe::Color& color) noexcept{
        if(!imageData.empty()){
            imageData.clear();
        }
        mWidth = width;
    ...
    void Image::setPixel(const fe::Vector2f& pos, const fe::Color& color) noexcept{
        setPixel(pos.x, pos.y, color);
    }
    fe::Vector2f Image::getSize() const noexcept{
        return fe::Vector2f(mWidth, mHeight);
    }
    void Image::setPixel(std::size_t x, std::size_t y, const fe::Color& color) noexcept{
        if(!imageData.empty() && x < mWidth && y < mHeight){
