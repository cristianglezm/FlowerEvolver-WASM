...
namespace fe{
    namespace priv{
        ...
        fe::Image generateNormalFromPetal(const fe::Image& sourceImage, const NoiseOptions& options) {
            fe::Vector2f sizeF = sourceImage.getSize();
            int width = static_cast<int>(sizeF.x);
            int height = static_cast<int>(sizeF.y);
            fe::Image normalMap;
            normalMap.create(width, height, fe::Color::Transparent);
            ...
            for(int y = 1; y < height - 1; ++y){
                for(int x = 1; x < width - 1; ++x){
                    ...
                    auto r = static_cast<fe::Uint8>((normal.x * 0.5f + 0.5f) * 255);
                    auto g = static_cast<fe::Uint8>((normal.y * 0.5f + 0.5f) * 255);
                    auto b = static_cast<fe::Uint8>((normal.z * 0.5f + 0.5f) * 255);
                    normalMap.setPixel(x, y, fe::Color(r, g, b));
                }
            }
            return normalMap;
        }
        ...
        float computeColorDistance(const fe::Color& a, const fe::Color& b) noexcept{
            float dr = (b.r - a.r) * (1.0f/255.f);
            float dg = (b.g - a.g) * (1.0f/255.f);
            float db = (b.b - a.b) * (1.0f/255.f);
            return std::sqrt(dr * dr + dg * dg + db * db);
        }
        float computeIntensity(const fe::Color& a, const fe::Color& b, const EmissiveOptions& opt) noexcept{
        ...
        fe::Image generateEmissiveFromPetal(const fe::Image& sourceImage, const EmissiveOptions& opt){
            ...
            int cx = W/2, cy = H/2;
            fe::Color centerColor = sourceImage.getPixel(cx, cy);
            fe::Image emissive;
            emissive.create(W, H, fe::Color::Black);
            for(int y = 0; y < H; ++y){
                for(int x = 0; x < W; ++x){
    ...
    void generatePetalLayer(
        ...
        if(params.useNormals){
            ...
            priv::NoiseOptions noiseOptions{};
            noiseOptions.numPoints = 32;
            noiseOptions.noiseMin = -14;
            noiseOptions.noiseMax = 64;
            noiseOptions.softNoise = true;
            noiseOptions.spread = 6;
            noiseOptions.softnessFactor = 1.1f;
            noiseOptions.directionBiasX = 0.2f;
            noiseOptions.directionBiasY = 0.5f;
            noiseOptions.scaleStrength = 0.5f;
            noiseOptions.frequency = 0.9f;
            noiseOptions.jitter = 0.4f;
            noiseOptions.noisePower = 0.8f;
            noiseOptions.baseHeight = 0.8f;

            auto noiseImage = priv::generateNormalFromPetal(
                petalLayerTexture,
                noiseOptions
            );
        ...
        if(params.useEmissive){
            priv::EmissiveOptions opts{};
            opts.colorThreshold = 0.8f;
            opts.minIntensity = 0.01f;
            opts.maxIntensity = 1.5f;
            opts.falloffPower = 1.2f;

            auto emissiveImage = priv::generateEmissiveFromPetal(
                petalLayerTexture,
                opts
            );
            ...
            for(const auto& pos : positions){
                ...
                if(srcColor == fe::Color::Black || srcColor.a == 0){
                    continue;
                }
