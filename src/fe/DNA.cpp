...
namespace fe{
	...
	: genomes(){
		...
		for(auto& g:arrGen){
			...
			if(!fe::detail::hasUsableOutputs(genomes.back(), 1)){
				throw std::invalid_argument("invalid DNA, genome has no usable output neurons");
			}
		}
	}
	...
	bool DNA::remove(EvoAI::Genome* g) noexcept{
		auto removed = std::remove_if(std::begin(genomes),std::end(genomes),
			[&](auto& g2){
				return (*g == g2);
			});
		bool isRemoved = (removed != std::end(genomes));
		genomes.erase(removed, std::end(genomes));
		return isRemoved;
	}
