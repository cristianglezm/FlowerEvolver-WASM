...
namespace fe{
	...
	: dna()
	, petals(){}
	Flower::Flower([[maybe_unused]] const fe::Vector2f& pos, int radius, int numLayers, float P, float bias)
	: dna()
	...
	Flower::Flower([[maybe_unused]] const fe::Vector2f& pos, int radius, int numLayers, float P, float bias, const Petals::Type& type)
	: dna()
	, petals(radius, numLayers, P, bias){
		//add stats genome so is compatible with EcoSystem Flowers
		dna.add(EvoAI::Genome(4,14,false,true));
	...
	Flower::Flower([[maybe_unused]] const fe::Vector2f& pos, int radius, int numLayers, float P, float bias, DNA&& dna, const Petals::Type& type)
	: dna(std::forward<DNA>(dna))
	, petals(radius, numLayers, P, bias){
		if(this->dna.size() < 2 || !fe::detail::hasUsableOutputs(this->dna[1], 4)){
			throw std::invalid_argument("invalid DNA, it should have 2 genomes");
		}
		draw(type, petals, this->dna[1]);
	}
	Flower::Flower([[maybe_unused]] const fe::Vector2f& pos, int radius, int numLayers, float P, float bias, DNA&& dna)
	: dna(std::forward<DNA>(dna))
	, petals(radius, numLayers, P, bias){
		if(this->dna.size() < 2 || !fe::detail::hasUsableOutputs(this->dna[1], 4)){
			throw std::invalid_argument("invalid DNA, it should have 2 genomes");
