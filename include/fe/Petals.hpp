...
namespace fe{
	...
	namespace priv{
		...
		*  @brief queries the neural network
		 ...
		 *  
		 *  @param [in] pos           fe::Vector2f& position
	 ...
	 */
	FE_API int getTimesDivisibleBy(int val, int divisor) noexcept;
	/**
	 ...
	 */
	FE_API void drawLayer(Petals& petals, EvoAI::Genome& g, int layer, bool applyLayeredRadiusScaling = true) noexcept;
	/**
	 ...
	 *  @param [in] g      a cppn EvoAI::Genome with 4 inputs 4 outputs
	 */
	FE_API void draw(Petals::Type t, Petals& petals, EvoAI::Genome& g) noexcept;
}
